...
interface StaggerProps {
  ...
  index: number;
  children: ReactNode;
  /** Fired once this block's entrance has landed (or immediately if skipped). Tells the dock the last block arrived. */
  onComplete?: () => void;
}

const ENTRANCE_BLUR_PX = 4;

// Cascade timing anchor. Each page resets it via <EntranceEpoch> on mount so it cascades on
// navigation, not only first load (else `now` is past every target on a later page and all skip).
const APP_MOUNT_MS = performance.now();
export const EntranceEpochContext = createContext(APP_MOUNT_MS);

/** Anchor the subtree's cascade to this mount, so its Staggers play on (re)visit. */
...
// Only skip after first paint, else a slow bundle parse pushes `now` past the targets and suppresses the cascade. Double-rAF = the frame after first paint.
...
interface UseStaggerEntranceOptions {
  ...
  index: number;
  /** Hold at the faded state until true: gates the cascade on async readiness so a section doesn't fade in empty. Defaults to true. */
  ready?: boolean;
}

type EntranceMotionProps = Pick<MotionProps, "initial" | "animate" | "transition">;

/** Motion props for a cascade entrance, so a component can drive its own root element and gate on `ready`. */
function useStaggerEntrance({
  index,
  ready = true,
}: UseStaggerEntranceOptions): EntranceMotionProps {
  // Lock readiness at mount so a late asset (false->true) still plays a fresh fade, not the skip shortcut.
...
export function Stagger({ index, children, onComplete }: StaggerProps) {
  ...
  // A skipped entrance (initial === false) has no animation to complete, so seed `done` true and report once on mount.
