...
- `true`: a translucent colour wash (`normal` blend). Works on any surface, needs no backdrop detection (deterministic, SSR-safe), but the band sits over the text so light text on a dark surface is muted.
