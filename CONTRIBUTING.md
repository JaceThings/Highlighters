# Contributing to highlighters

Thanks for your interest in highlighters. This guide covers how to set up the project locally, the commands you'll use day to day, and the process for getting changes merged and released.
...
- `pnpm size`: check the per-package bundle-size budgets
- `pnpm knip`: check for unused files, dependencies, and exports
- `pnpm consumer-smoke`: pack the tarballs and run the ESM/CJS/subpath/SSR import smokes

Run a script in a single package with `pnpm --filter @highlighters/core <script>`.
...
Example: `feat/glyph-snap-mode`.

## Changesets

highlighters uses [Changesets](https://github.com/changesets/changesets) to manage versions and changelogs.
